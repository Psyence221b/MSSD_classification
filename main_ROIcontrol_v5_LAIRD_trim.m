...
for nSubj = 1:size(BasicInfo,1)
    ...
    Data_areas = [Data_mPFC Data_BLA Data_Amyg Data_limbic Data_motor Data_visual Data_dmn Data_auditory];
    ...
    % mPFC-limbic
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_mPFC_limbic(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(1,4,:)),2);']); 
    % mPFC-motor
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_mPFC_motor(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(1,5,:)),2);']);     
    % mPFC-visual
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_mPFC_visual(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(1,6,:)),2);']); 
    % mPFC-dmn
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_mPFC_dmn(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(1,7,:)),2);']); 
    % mPFC-auditory
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_mPFC_auditory(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(1,8,:)),2);']); 
    ...
    % BLA-limbic
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_BLA_limbic(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(2,4,:)),2);']); 
    % BLA-motor
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_BLA_motor(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(2,5,:)),2);']);     
    % BLA-visual
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_BLA_visual(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(2,6,:)),2);']); 
    % BLA-dmn
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_BLA_dmn(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(2,7,:)),2);']); 
    % BLA-auditory
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_BLA_auditory(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(2,8,:)),2);']); 
    
    % Amyg-limbic
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_Amyg_limbic(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(3,4,:)),2);']); 
    % Amyg-motor
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_Amyg_motor(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(3,5,:)),2);']);     
    % Amyg-visual
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_Amyg_visual(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(3,6,:)),2);']); 
    % Amyg-dmn
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_Amyg_dmn(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(3,7,:)),2);']); 
    % Amyg-auditory
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_Amyg_auditory(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(3,8,:)),2);']); 
...
%% Save temporaly
save TMP_MSSD_ROIcontrol_v5_trim
...
%% Make Output Table
ResultTable_TR50sw      = addvars(BasicInfo, TR50sw_mPFC_BLA, TR50sw_mPFC_Amyg, TR50sw_mPFC_limbic, TR50sw_mPFC_motor, TR50sw_mPFC_visual, TR50sw_mPFC_dmn, TR50sw_mPFC_auditory, TR50sw_BLA_Amyg, TR50sw_BLA_limbic, TR50sw_BLA_motor, TR50sw_BLA_visual, TR50sw_BLA_dmn, TR50sw_BLA_auditory, TR50sw_Amyg_limbic, TR50sw_Amyg_motor, TR50sw_Amyg_visual, TR50sw_Amyg_dmn, TR50sw_Amyg_auditory);
ResultTable_TR40sw      = addvars(BasicInfo, TR40sw_mPFC_BLA, TR40sw_mPFC_Amyg, TR40sw_mPFC_limbic, TR40sw_mPFC_motor, TR40sw_mPFC_visual, TR40sw_mPFC_dmn, TR40sw_mPFC_auditory, TR40sw_BLA_Amyg, TR40sw_BLA_limbic, TR40sw_BLA_motor, TR40sw_BLA_visual, TR40sw_BLA_dmn, TR40sw_BLA_auditory, TR40sw_Amyg_limbic, TR40sw_Amyg_motor, TR40sw_Amyg_visual, TR40sw_Amyg_dmn, TR40sw_Amyg_auditory); 
ResultTable_TR30sw      = addvars(BasicInfo, TR30sw_mPFC_BLA, TR30sw_mPFC_Amyg, TR30sw_mPFC_limbic, TR30sw_mPFC_motor, TR30sw_mPFC_visual, TR30sw_mPFC_dmn, TR30sw_mPFC_auditory, TR30sw_BLA_Amyg, TR30sw_BLA_limbic, TR30sw_BLA_motor, TR30sw_BLA_visual, TR30sw_BLA_dmn, TR30sw_BLA_auditory, TR30sw_Amyg_limbic, TR30sw_Amyg_motor, TR30sw_Amyg_visual, TR30sw_Amyg_dmn, TR30sw_Amyg_auditory);

ResultTable_TR50tsw      = addvars(BasicInfo, TR50tsw_mPFC_BLA, TR50tsw_mPFC_Amyg, TR50tsw_mPFC_limbic, TR50tsw_mPFC_motor, TR50tsw_mPFC_visual, TR50tsw_mPFC_dmn, TR50tsw_mPFC_auditory, TR50tsw_BLA_Amyg, TR50tsw_BLA_limbic, TR50tsw_BLA_motor, TR50tsw_BLA_visual, TR50tsw_BLA_dmn, TR50tsw_BLA_auditory, TR50tsw_Amyg_limbic, TR50tsw_Amyg_motor, TR50tsw_Amyg_visual, TR50tsw_Amyg_dmn, TR50tsw_Amyg_auditory);
ResultTable_TR40tsw      = addvars(BasicInfo, TR40tsw_mPFC_BLA, TR40tsw_mPFC_Amyg, TR40tsw_mPFC_limbic, TR40tsw_mPFC_motor, TR40tsw_mPFC_visual, TR40tsw_mPFC_dmn, TR40tsw_mPFC_auditory, TR40tsw_BLA_Amyg, TR40tsw_BLA_limbic, TR40tsw_BLA_motor, TR40tsw_BLA_visual, TR40tsw_BLA_dmn, TR40tsw_BLA_auditory, TR40tsw_Amyg_limbic, TR40tsw_Amyg_motor, TR40tsw_Amyg_visual, TR40tsw_Amyg_dmn, TR40tsw_Amyg_auditory); 
ResultTable_TR30tsw      = addvars(BasicInfo, TR30tsw_mPFC_BLA, TR30tsw_mPFC_Amyg, TR30tsw_mPFC_limbic, TR30tsw_mPFC_motor, TR30tsw_mPFC_visual, TR30tsw_mPFC_dmn, TR30tsw_mPFC_auditory, TR30tsw_BLA_Amyg, TR30tsw_BLA_limbic, TR30tsw_BLA_motor, TR30tsw_BLA_visual, TR30tsw_BLA_dmn, TR30tsw_BLA_auditory, TR30tsw_Amyg_limbic, TR30tsw_Amyg_motor, TR30tsw_Amyg_visual, TR30tsw_Amyg_dmn, TR30tsw_Amyg_auditory);


% Split Variable
ResultTable_TR50sw      = splitvars(ResultTable_TR50sw,[21:38]);
ResultTable_TR40sw      = splitvars(ResultTable_TR40sw,[21:38]);
ResultTable_TR30sw      = splitvars(ResultTable_TR30sw,[21:38]);
ResultTable_TR50tsw     = splitvars(ResultTable_TR50tsw,[21:38]);
ResultTable_TR40tsw     = splitvars(ResultTable_TR40tsw,[21:38]);
ResultTable_TR30tsw     = splitvars(ResultTable_TR30tsw,[21:38]);
...
%% Save Results
cd(Dirdata); cd('..');
mkdir('Result_MSSD_ROIcontrol_v5(LAIRD)_trim');
cd('Result_MSSD_ROIcontrol_v5(LAIRD)_trim');
