...
%     File_BLA = dir('*_BLA_*'); File_BLA = File_BLA.name;
    ...
    Flip_Data_VTA = [flip(Data_VTA); Data_VTA; flip(Data_VTA)];
    Flip_Data_Insula = [flip(Data_Insula); Data_Insula; flip(Data_Insula)];
    TMP_Data_areas = [Flip_Data_mPFC Flip_Data_BLA Flip_Data_Amyg Flip_Data_VTA Flip_Data_Insula];
    ...
    clear File_BLA File_mPFC File_whole formatSpec fileID Flip_Data* L
    ...
    [Raw_VTA(nSubj,1:5)] = calculate_5variables(Data_VTA,1);
    [Raw_Insular(nSubj,1:5)] = calculate_5variables(Data_Insula,1);
...
%% Save Results
cd(Dirdata); cd('..');
mkdir('Result_MSSD_downsampling');
cd('Result_MSSD_downsampling');
