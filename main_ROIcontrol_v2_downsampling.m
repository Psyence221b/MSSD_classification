...
for nSubj = 1:size(BasicInfo,1)
    ...
    Flip_Data_mPFC = [flip(Data_mPFC); Data_mPFC; flip(Data_mPFC)];
    Flip_Data_BLA = [flip(Data_BLA); Data_BLA; flip(Data_BLA)];
    Flip_Data_Amyg = [flip(Data_Amyg); Data_Amyg; flip(Data_Amyg)];
    Flip_Data_calcarine = [flip(Data_calcarine); Data_calcarine; flip(Data_calcarine)];
    Flip_Data_heschl = [flip(Data_heschl); Data_heschl; flip(Data_heschl)];
    Flip_Data_postcentral = [flip(Data_postcentral); Data_postcentral; flip(Data_postcentral)];
    Flip_Data_olfactory = [flip(Data_olfactory); Data_olfactory; flip(Data_olfactory)];
    TMP_Data_areas = [Flip_Data_mPFC Flip_Data_BLA Flip_Data_Amyg Flip_Data_calcarine Flip_Data_heschl Flip_Data_postcentral Flip_Data_olfactory];

    % Downsampling to TR = 3sec
    TR_orig = BasicInfo.TRSec(nSubj);
    L = ceil(length(Data_mPFC)*(floor(TR_orig*1000))/(floor(TR_down*1000)));
    TMP_Data_areas = resample(TMP_Data_areas,floor(TR_orig*1000),floor(TR_down*1000));
    Data_areas = TMP_Data_areas([L:2*L-1],:);
    
    clear File_BLA File_mPFC File_whole formatSpec fileID TMP_Data_areas Flip_Data*
...
%% Save temporaly
save TMP_MSSD_ROIcontrol2_downsampling
...
%% Save Results
cd(Dirdata); cd('..');
mkdir('Result_MSSD_ROIcontrol_v2_downsampling');
cd('Result_MSSD_ROIcontrol_v2_downsampling');
