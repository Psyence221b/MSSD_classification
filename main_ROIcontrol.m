...
for nSubj = 1:size(BasicInfo,1)
    ...
    Data_olfactory = Data_whole(:,45);
    Data_tempinf = Data_whole(:,47);
    
    Data_areas = [Data_mPFC Data_BLA Data_Amyg Data_calcarine Data_heschl Data_postcentral Data_olfactory Data_tempinf];
    ...
    [Raw_cuneus(nSubj,1:5)] = calculate_5variables(Data_heschl,1);
    [Raw_tempsup(nSubj,1:5)] = calculate_5variables(Data_postcentral,1);
    [Raw_tempmid(nSubj,1:5)] = calculate_5variables(Data_olfactory,1);
    [Raw_tempinf(nSubj,1:5)] = calculate_5variables(Data_tempinf,1);
    ...
    TMP_Dfc_sw(Idx_nan) = [];
    TMP_Dfc_sw = reshape(TMP_Dfc_sw,8,8,[]);
    ...
    TMP_Dfc_tsw(Idx_nan) = [];
    TMP_Dfc_tsw = reshape(TMP_Dfc_tsw,8,8,[]);
    ...
    % mPFC-cuneus
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_mPFC_cuneus(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(1,5,:)),2);']);     
    % mPFC-tempsup
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_mPFC_tempsup(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(1,6,:)),2);']); 
    % mPFC-tempmid
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_mPFC_tempmid(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(1,7,:)),2);']); 
    % mPFC-tempinf
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_mPFC_tempinf(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(1,8,:)),2);']); 
    ...
    % BLA-cuneus
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_BLA_cuneus(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(2,5,:)),2);']);     
    % BLA-tempsup
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_BLA_tempsup(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(2,6,:)),2);']); 
    % BLA-tempmid
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_BLA_tempmid(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(2,7,:)),2);']); 
    % BLA-tempinf
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_BLA_tempinf(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(2,8,:)),2);']); 
    ...
    % Amyg-cuneus
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_Amyg_cuneus(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(3,5,:)),2);']);     
    % Amyg-tempsup
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_Amyg_tempsup(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(3,6,:)),2);']); 
    % Amyg-tempmid
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_Amyg_tempmid(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(3,7,:)),2);']); 
    % Amyg-tempinf
    eval(['[TR' num2str(windowsize) Var_methods{nMethod} '_Amyg_tempinf(nSubj,1:5)] = calculate_5variables(squeeze(TMP_Dfc_' Var_methods{nMethod} '(3,8,:)),2);']); 
...
%% Make Output Table
ResultTable_TR50sw      = addvars(BasicInfo, TR50sw_mPFC_BLA, TR50sw_mPFC_Amyg, TR50sw_mPFC_calcarine, TR50sw_mPFC_cuneus, TR50sw_mPFC_tempsup, TR50sw_mPFC_tempmid, TR50sw_mPFC_tempinf, TR50sw_BLA_Amyg, TR50sw_BLA_calcarine, TR50sw_BLA_cuneus, TR50sw_BLA_tempsup, TR50sw_BLA_tempmid, TR50sw_BLA_tempinf, TR50sw_Amyg_calcarine, TR50sw_Amyg_cuneus, TR50sw_Amyg_tempsup, TR50sw_Amyg_tempmid, TR50sw_Amyg_tempinf);
ResultTable_TR40sw      = addvars(BasicInfo, TR40sw_mPFC_BLA, TR40sw_mPFC_Amyg, TR40sw_mPFC_calcarine, TR40sw_mPFC_cuneus, TR40sw_mPFC_tempsup, TR40sw_mPFC_tempmid, TR40sw_mPFC_tempinf, TR40sw_BLA_Amyg, TR40sw_BLA_calcarine, TR40sw_BLA_cuneus, TR40sw_BLA_tempsup, TR40sw_BLA_tempmid, TR40sw_BLA_tempinf, TR40sw_Amyg_calcarine, TR40sw_Amyg_cuneus, TR40sw_Amyg_tempsup, TR40sw_Amyg_tempmid, TR40sw_Amyg_tempinf); 
ResultTable_TR30sw      = addvars(BasicInfo, TR30sw_mPFC_BLA, TR30sw_mPFC_Amyg, TR30sw_mPFC_calcarine, TR30sw_mPFC_cuneus, TR30sw_mPFC_tempsup, TR30sw_mPFC_tempmid, TR30sw_mPFC_tempinf, TR30sw_BLA_Amyg, TR30sw_BLA_calcarine, TR30sw_BLA_cuneus, TR30sw_BLA_tempsup, TR30sw_BLA_tempmid, TR30sw_BLA_tempinf, TR30sw_Amyg_calcarine, TR30sw_Amyg_cuneus, TR30sw_Amyg_tempsup, TR30sw_Amyg_tempmid, TR30sw_Amyg_tempinf);

ResultTable_TR50tsw      = addvars(BasicInfo, TR50tsw_mPFC_BLA, TR50tsw_mPFC_Amyg, TR50tsw_mPFC_calcarine, TR50tsw_mPFC_cuneus, TR50tsw_mPFC_tempsup, TR50tsw_mPFC_tempmid, TR50tsw_mPFC_tempinf, TR50tsw_BLA_Amyg, TR50tsw_BLA_calcarine, TR50tsw_BLA_cuneus, TR50tsw_BLA_tempsup, TR50tsw_BLA_tempmid, TR50tsw_BLA_tempinf, TR50tsw_Amyg_calcarine, TR50tsw_Amyg_cuneus, TR50tsw_Amyg_tempsup, TR50tsw_Amyg_tempmid, TR50tsw_Amyg_tempinf);
ResultTable_TR40tsw      = addvars(BasicInfo, TR40tsw_mPFC_BLA, TR40tsw_mPFC_Amyg, TR40tsw_mPFC_calcarine, TR40tsw_mPFC_cuneus, TR40tsw_mPFC_tempsup, TR40tsw_mPFC_tempmid, TR40tsw_mPFC_tempinf, TR40tsw_BLA_Amyg, TR40tsw_BLA_calcarine, TR40tsw_BLA_cuneus, TR40tsw_BLA_tempsup, TR40tsw_BLA_tempmid, TR40tsw_BLA_tempinf, TR40tsw_Amyg_calcarine, TR40tsw_Amyg_cuneus, TR40tsw_Amyg_tempsup, TR40tsw_Amyg_tempmid, TR40tsw_Amyg_tempinf); 
ResultTable_TR30tsw      = addvars(BasicInfo, TR30tsw_mPFC_BLA, TR30tsw_mPFC_Amyg, TR30tsw_mPFC_calcarine, TR30tsw_mPFC_cuneus, TR30tsw_mPFC_tempsup, TR30tsw_mPFC_tempmid, TR30tsw_mPFC_tempinf, TR30tsw_BLA_Amyg, TR30tsw_BLA_calcarine, TR30tsw_BLA_cuneus, TR30tsw_BLA_tempsup, TR30tsw_BLA_tempmid, TR30tsw_BLA_tempinf, TR30tsw_Amyg_calcarine, TR30tsw_Amyg_cuneus, TR30tsw_Amyg_tempsup, TR30tsw_Amyg_tempmid, TR30tsw_Amyg_tempinf);
...
%% Save Results
cd(Dirdata); cd('..');
mkdir('Result_MSSD_ROIcontrol');
cd('Result_MSSD_ROIcontrol');
